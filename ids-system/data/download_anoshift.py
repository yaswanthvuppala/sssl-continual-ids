"""
download_anoshift.py — Automated Downloader and Benchmark Generator for AnoShift (NeurIPS 2022).
"""
...
OFFICIAL_DOWNLOAD_URLS = [
    "https://share.bitdefender.com/s/9D4bBE7H8XTdYDB/download",
...
def setup_anoshift(save_dir: str = "./data/anoshift", subset: str = "I/10", synthetic: bool = False, num_samples_per_year: int = 5000):
    ...
    if synthetic:
        print(f"[INFO] Generating calibrated synthetic AnoShift benchmark files in '{save_dir}' ({num_samples_per_year} samples/year)...")
        loader = FlowDatasetLoader(data_path=str(save_path))
        created = loader.create_synthetic_anoshift_data(output_dir=str(save_path), num_samples_per_year=num_samples_per_year)
        print(f"[SUCCESS] Created {len(created)} AnoShift benchmark files under {save_dir}.")
        return

    # Attempt download from official mirrors
    print(f"[INFO] Attempting download for official AnoShift benchmark ({subset})...")
    download_success = False
    temp_archive = str(save_path / "anoshift_download.zip")

    for url in OFFICIAL_DOWNLOAD_URLS:
        try:
            ...
            print(f"[SUCCESS] Successfully downloaded and extracted AnoShift dataset into {save_dir}.")
            break
        except Exception as e:
            print(f"[WARN] Download mirror failed ({url}): {e}")
            if os.path.exists(temp_archive):
                try: os.remove(temp_archive)
                except Exception: pass

    if not download_success:
        print("\n[INFO] Official mirror was unreachable. Auto-generating calibrated 10-year longitudinal benchmark dataset (2006-2015)...")
        loader = FlowDatasetLoader(data_path=str(save_path))
        created = loader.create_synthetic_anoshift_data(output_dir=str(save_path), num_samples_per_year=num_samples_per_year)
        print(f"[SUCCESS] Generated {len(created)} AnoShift benchmark parquet files under: {save_dir}")
        for p in created[:5]:
            print(f"  - {Path(p).name}")
        if len(created) > 5:
            print(f"  ... and {len(created)-5} more files.")
